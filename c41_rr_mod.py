...
def makeModel():
    ...
    **b** has a negative feedback onto **a**.
    ...
    e3.Km = 0.1#*a.vec.conc
    e3.kcat = 1

    #moose.connect( e4, 'sub', a, 'reac')
    #moose.connect( e4, 'prd', b, 'reac')
    #moose.connect( s, 'nOut', e4, 'enzDest')
    #e4.Km=0
    #e4.kcat=0 
...
#    img = mpimg.imread( 'turingPatternTut.png' )
  ...
  #  imgplot = plt.imshow( img )
    ...
    plt.legend()
    fig.canvas.draw()
    e3=moose.element('/model/compartment/e3')
    print e3.vec.Km
